...
class Admin extends CI_Controller
{
    ...
    public function guardar_anamnesis_to(){
        try {
            $data['MotivoEntrevista'] = $this->input->post('MotivoEntrevista');
            $data['ProblematicaActual'] = $this->input->post('ProblematicaActual');
            $data['HistoriaFamiliar'] = $this->input->post('HistoriaFamiliar');
            $data['DesarrolloNatal'] = $this->input->post('DesarrolloNatal');
            $data['DesarrolloOntogenetico'] = $this->input->post('DesarrolloOntogenetico');
            $data['IngresoMaternalPreesciolarEscolar'] = $this->input->post('IngresoMaternalPreesciolarEscolar');
            $data['aluTurnoATencion'] = $this->input->post('aluTurnoATencion');
            $data['EspecialistaAsignado'] = $this->input->post('EspecialistaAsignado');
            $data['EscuelaMunicipal'] = $this->input->post('EscuelaMunicipal');
            $data['EscuelaOtrosMunicipios'] = $this->input->post('EscuelaOtrosMunicipios');
            $data['NombreEscuelaOtrosMunicipios'] = $this->input->post('NombreEscuelaOtrosMunicipios');
            $data['NombreEscuelaEstadal'] = $this->input->post('NombreEscuelaEstadal');
            $data['id_inscripcion'] = $this->input->post('id_inscripcion');

            $this->db->insert('anamnesis_to', $data);
    ...
    function student_new($param1 = '', $param2 = '' , $param3 = '')
    {
            ...
            $id_school = $this->config->item('id_school');
            $page_data['grados'] = VistaGradoEscuela::where('id_escuela', '=', 2)
                                                ->orderBy('id_grado')
                                                ->groupBy('id_grado')
                                                ->get();
            $page_data['estados'] = Estado::orderBy('nombre')->get();
    ...
    function student_certificate($param1 = '')
    {
            ...
            $this->pdf->SetFont('Arial', '', 8);
            $this->pdf->Cell(10);
            $this->pdf->Cell(0,4,iconv('UTF-8','windows-1252','Residente del Municipio:     '.$residente),0,50,'L');
    ...
    function student_referred($param1 = '')
    {
            ...
            $page_data['page_title'] = "ESTUDIANTES REFERIDOS DESDE UEM";
    ...
    function get_class_section($class_id)
    {
        ...
        $sections = $this->db->get_where('vescuela_seccion' , array('id_grado' => $class_id, 'id_escuela' => 2))->result_array();
