jQuery( document ).ready(function() {
	...
	/*** CENSUS EDIT ESPECIALISTA ***/

	jQuery('#siguiente-datos-interes-dos').click(function(e){
		var data = jQuery('#form-general').serialize()+ '&' +
		jQuery('#form-datos-alumno').serialize()+ '&' +
		jQuery('#form-datos-representante').serialize()+ '&' +
		jQuery('#form-datos-madre').serialize()+ '&' +
		...
		if (jQuery('#form-general').valid() && jQuery('#form-datos-alumno').valid() && jQuery('#form-datos-representante').valid()
		   ...
		   && jQuery('#form-datos-saludalu').valid()){
			if (jQuery('#form-datos-otros').valid()) {
				jQuery('#siguiente-datos-interes-dos').prop("disabled",true);
				window.scrollTo(0,0);
				AjaxEditarCenso('editar_censo',data,'POST',true,function(response){
				    mostrarModal("Éxito", "Censo actualizado con éxito.", 'type-primary',
			                [{
			                    label: 'Aceptar',
			                    cssClass: 'btn-success',
			                    action: function(dialogItself){
				    ...
				    jQuery('#siguiente-datos-interes-dos').prop("disabled",false);
				});
			} else {
				jQuery('#siguiente-datos-interes-dos').prop("disabled",false);
				validator.focusInvalid();
	...
	jQuery('#btnSaveMainDos').click(function(e){
		var data = jQuery('#form-general').serialize()+ '&' +
		jQuery('#form-datos-alumno').serialize()+ '&' +
		jQuery('#form-datos-representante').serialize()+ '&' +
		jQuery('#form-datos-madre').serialize()+ '&' +
		...
		if (jQuery('#form-general').valid() && jQuery('#form-datos-alumno').valid() && jQuery('#form-datos-representante').valid()
		   ...
		   && jQuery('#form-datos-saludalu').valid()){
			if (jQuery('#form-datos-otros').valid()) {
				jQuery('#btnSaveMainDos').prop("disabled",true);
				window.scrollTo(0,0);
				AjaxEditarCenso('editar_censo',data,'POST',true,function(response){
				    mostrarModal("Éxito", "Censo actualizado con éxito.", 'type-primary',
			                [{
				    ...
				    jQuery('#btnSaveMainDos').prop("disabled",false);
				});
			} else {
				jQuery('#btnSaveMainDos').prop("disabled",false);
				validator.focusInvalid();
