jQuery( document ).ready(function() {
	...
	jQuery( '#GradoRepetido' ).change(function(e) {

	  	if (parseInt(jQuery('#GradoRepetido').val()) > 9){
	  		jQuery('#div-materia-repite').show();
	  	}else{
	  		jQuery('#div-materia-repite').hide();
	  	}
	});
	...
	jQuery('.materia-pendiente').click(function(e) {
		if (this.value != 0) {
            ...
            jQuery('#MateriasPendientes').change(function(e) {
                        if (jQuery(this).val().length > 2) {
                                    alert('Solo puede seleccionar 2 materias pendientes.');
