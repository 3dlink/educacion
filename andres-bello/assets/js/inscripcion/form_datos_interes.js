jQuery( document ).ready(function() {
				    ...
				    mostrarModal("Éxito", "Alumno registrado con éxito, ha sido enviado el resumen via correo electrónio.", 'type-primary',
			                [{
			                    ...
			                    action: function(dialogItself){
			                        ...
			                        dialogItself.close();
			                        var enviar_correo = confirm("¿Desea enviar el certificado y carnet via correo electrónico?");
			                        mostrarReporte(response, enviar_correo);
			                        mostrarCarnet(response, enviar_correo);
			                    }
			                }]);
				    ...
				    mostrarModal("Éxito", "Alumno registrado con éxito, ha sido enviado el resumen via correo electrónio.", 'type-primary',
			                [{
			                    ...
			                    action: function(dialogItself){
			                        dialogItself.close();
					var enviar_correo = confirm("¿Desea enviar el certificado y carnet via correo electrónico?");
			                        mostrarReporteAdd(response, enviar_correo);
			                        mostrarCarnetAdd(response, enviar_correo);
	...
	function mostrarReporteAdd(respuesta, enviar_correo){
		var respuestaCenso = JSON.parse(respuesta).id_inscripcion;
		AjaxReportEdit('../../../ResumenInscripcion',respuestaCenso,enviar_correo,'POST',true,function(response){
			alert(response);
		});
	}

	function mostrarCarnetAdd(respuesta, enviar_correo){
		var respuestaCenso = JSON.parse(respuesta).id_inscripcion;
		AjaxReportEmpty('../../../Carnet',respuestaCenso,enviar_correo,'POST',true,function(response){
			alert(response);
		});
	}

	function mostrarReporte(respuesta, enviar_correo){
		var respuestaCenso = JSON.parse(respuesta).id_inscripcion;
		AjaxReport('../../ResumenInscripcion',respuestaCenso,enviar_correo,'POST',true,function(response){
			alert(response);
		});
	}

	function mostrarCarnet(respuesta, enviar_correo){
		var respuestaCenso = JSON.parse(respuesta).id_inscripcion;
		AjaxReportEmpty('../../Carnet',respuestaCenso,enviar_correo,'POST',true,function(response){
			alert(response);
		});
