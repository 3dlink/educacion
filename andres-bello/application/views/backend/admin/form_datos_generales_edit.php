    ...
    <div class="box-content">
        <form id="form-general" class="form-horizontal form-groups-bordered form-censo" accept-charset="utf-8">
            ...
            <div id="div-GradoRepetido" class="form-group" style="display:none">
                ...
                <div class="col-sm-6">
                  <select class="form-control" id="GradoRepetido" name="GradoRepetido" required onchange="return get_class_subject_repeat(this.value)">>
                    <option value="0" selected="selected" disabled="disabled">Seleccione...</option>
                    <?php foreach ($grados as $grado){ ?>
            ...
            <div class="form-group" id="div-materia-repite" style="display:none">
                <label for="address" class="col-sm-4 control-label">Especifique con cuáles materias repite</label>
                <div class="col-sm-6">
                        <select id="MateriasRepite" name="MateriasRepite" class="form-control" style="width:100%;" multiple>
                            <option value=""><?php echo "Seleccione primero el grado"?></option>
                        </select>
                </div>
            </div>
            <div class="form-group depende-PrimerGrupo depende-EstudioAnterior" id="div-materia-pendiente" style="<?php echo ($grado['id_grado'] > 9) ? 'display: none;':'' ?>">
            ...
            <div class="form-group div-materia-pendiente" style="display:none">
                ...
                <div class="col-sm-6">
                        <select id="MateriasPendientes" name="MateriasPendientes" class="form-control" style="width:100%;" multiple>
                            <option value=""><?php echo "Seleccione primero el grado"?></option>
                        </select>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="col-sm-6">
                  <select class="form-control" name="SeccionACursar" required id="section_selection_holder">
                    <option value="0"><?php echo "Seleccione primero el grado"?></option>
                  </select>
                </div>
            </div>
            <div class="form-group">
...
<script type="text/javascript">
            ...
            function get_class_section(class_id) {
                        $.ajax({
                                    url: '<?php echo base_url().$this->session->userdata('login_type');?>/get_quota_by_school_grade/' + class_id ,
                                    success: function(response)
                                    {
                                                if (response <= 2) {
                                                            if (response < 0) {
                                                                        alert('La matrícula de este grado esta completa, por favor seleccione otro grado');
                                                                        location.reload();
                                                            } else {
                                                                        alert('La matrícula de este grado esta por completarse');
                                                                        $.ajax({
            ...
            function get_class_subject(class_id) {
                        $.ajax({
                                    ...
                                    success: function(response)
                                    {
                                                jQuery('#MateriasPendientes').html(response);
                                    }
                        });
            }

            function get_class_subject_repeat(class_id) {
                        $.ajax({
                                    url: '<?php echo base_url().$this->session->userdata('login_type');?>/get_class_subject/' + class_id ,
                                    success: function(response)
                                    {
                                                jQuery('#MateriasRepite').html(response);
                                    }
                        });
            }
</script>